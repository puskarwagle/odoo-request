...
class ReqTopicLink(models.Model):
    ...
    rquest_request_state = fields.Selection(
    ...
    def _onchange_amount(self):
        ...
        if self.amount > self.remaining_amount:
            warning = {
                'title': 'Warning!',
                'message': 'Amount is greater than the remaining amount.',
            }
    ...
    def _compute_remaining_amount(self):
        for record in self:
            if record.request_id.request_state == 'approved':
                ...
                record.remaining_amount = record.topic_id.remaining_amount if record.topic_id.remaining_amount else record.max_amount
